  ...
  return (
    ...
    >
      <div className="shell">
        <div className="relative flex items-center justify-between gap-6">
          ...
          <nav aria-label="Primary" className="absolute left-1/2 hidden -translate-x-1/2 lg:block">
            <ul className={cn("flex items-center transition-[gap] duration-500 ease-out", scrolled ? "gap-8" : "gap-10")}>
              {NAV.map((item) => {
          ...
          <button
            ...
            className={cn(
              ...
              light ? "border-ink-line bg-ink-foreground/10 backdrop-blur" : "border-border bg-card/70 hover:border-brand/50",
            )}
          >
      ...
      <div
        ...
        aria-hidden={!open}
      >
        <nav aria-label="Mobile" className="shell flex h-full flex-col justify-center gap-2 pb-16">
          {NAV.map((item, index) => {
            const targetId = item.href.slice(1);
            return (
              <a
